...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // DATA MUTLAK SESUAI LAPORAN FINAL & PERMENKES 2023
    $mf = [
        "Suhu" => [
...
<head>
    ...
    <title>SPK Fuzzy Sugeno Orde-0 | Kelayakan Air</title>
    ...
    <style>
        ...
        .hero {
            background: linear-gradient(135deg, #d63384 0%, #f7b7c1 100%);
...
<div class="app-shell p-3 p-md-4">
    <div class="hero mb-4">
        <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
            <div>
                <span class="badge pill mb-2">FIS Sugeno Orde-0 • Permenkes 2023 • 3D Control Surface</span>
                <h1 class="h3 fw-bold mb-2">SPK Kelayakan Kualitas Air Pemandian Umum</h1>
                <p class="mb-0 opacity-90">Sistem Pendukung Keputusan untuk menganalisis kualitas air pemandian umum menggunakan logika fuzzy.</p>
            </div>
            ...
            <div class="<?= $result ? 'col-lg-5' : 'col-12' ?>">
                <div class="cardx p-4 sticky-panel form-card h-100">
                    ...
                    <form method="post">
                        ...
                        <button class="btn btn-primary w-100">Analisa Kelayakan</button>
                    </form>
                </div>
            </div>
            <?php if ($result): ?>
...
z* = Σ(αᵢ × cᵢ) / Σ(αᵢ)
            ...
            <script>
                // FUNGSI UNTUK GENERATE 3D CONTROL SURFACE VIA PLOTLY
                function fuzzyEngineJS(T, C, pH, DO) {
                    ...
                    let mfS = {
                        "Dingin": t_trap(T, 15, 15, 20, 25), "Sejuk": t_tri(T, 20, 25, 30), "Hangat": t_trap(T, 25, 30, 35, 35)
                    };
                    let mfC = {
                        "Keruh": t_trap(C, 0, 0, 1.2, 1.6), "Sedang": t_trap(C, 1.2, 1.6, 2.2, 2.6), "Jernih": t_trap(C, 2.2, 2.6, 3, 3)
                    };
                    let mfP = {
                        "Asam": t_trap(pH, 4, 4, 4.5, 5.0), "Normal": t_trap(pH, 4.5, 5.0, 9.0, 9.5), "Basa": t_trap(pH, 9.0, 9.5, 10, 10)
                    };
                    let mfD = {
                        "Rendah": t_trap(DO, 0, 0, 3.5, 4.5), "Optimal": t_trap(DO, 3.5, 4.5, 7.0, 8.0), "Tinggi": t_trap(DO, 7.0, 8.0, 10, 10)
                    };
                ...
                function renderAllMF() {
                    mfCharts = [];
                    // DATA UPDATE: Permenkes 2023 Boundaries
                    mfCharts.push(createMFChart('mfSuhu', 15, 35, <?= $result['input']['T'] ?>, {
                        'Dingin': { color: '#1463ff', fn: (x) => trapmf(x, 15, 15, 20, 25) },
                        'Sejuk': { color: '#198754', fn: (x) => trimf(x, 20, 25, 30) },
                        'Hangat': { color: '#fd7e14', fn: (x) => trapmf(x, 25, 30, 35, 35) }
                    }));

                    mfCharts.push(createMFChart('mfKekeruhan', 0, 3, <?= $result['input']['C'] ?>, {
                        'Keruh': { color: '#dc3545', fn: (x) => trapmf(x, 0, 0, 1.2, 1.6) },
                        'Sedang': { color: '#198754', fn: (x) => trapmf(x, 1.2, 1.6, 2.2, 2.6) },
                        'Jernih': { color: '#1463ff', fn: (x) => trapmf(x, 2.2, 2.6, 3, 3) }
                    }));

                    mfCharts.push(createMFChart('mfPH', 4, 10, <?= $result['input']['pH'] ?>, {
                        'Asam': { color: '#dc3545', fn: (x) => trapmf(x, 4, 4, 4.5, 5.0) },
                        'Normal': { color: '#198754', fn: (x) => trapmf(x, 4.5, 5.0, 9.0, 9.5) },
                        'Basa': { color: '#dc3545', fn: (x) => trapmf(x, 9.0, 9.5, 10, 10) }
                    }));

                    mfCharts.push(createMFChart('mfDO', 0, 10, <?= $result['input']['DO'] ?>, {
                        'Rendah': { color: '#dc3545', fn: (x) => trapmf(x, 0, 0, 3.5, 4.5) },
                        'Optimal': { color: '#198754', fn: (x) => trapmf(x, 3.5, 4.5, 7.0, 8.0) },
                        'Tinggi': { color: '#1463ff', fn: (x) => trapmf(x, 7.0, 8.0, 10, 10) }
                    }));
